"""Author: Asaf Ben-Zur
Best used for: 
This module is deprecated. Please use CurveZipB.
"""
...
def construct(mansur, MnsBuildModule, **kwargs):
	...
	if bindCurves[0] and bindCurves[1]:
		...
		uiCtrlsGrp = mnsUtils.createNodeReturnNameStd(parentNode = mainCtrl, side =  rootGuide.side, body = rootGuide.body + "UICtrls", alpha = rootGuide.alpha, id =  rootGuide.id, buildType = "group", incrementAlpha = False)
		pm.delete(pm.pointConstraint(cornerB, uiCtrlsGrp.node))
		...
		pm.parent(upperLidOffsetGrp.node, uiCtrlsGrp.node)
		...
		pm.parent(lowerLidOffsetGrp.node, uiCtrlsGrp.node)
		...
		setRangeNode = mnsNodes.setRangeNode([1.0, 1.0, 1.0],
												...
												[mnsCurveZipNode.node.AToMid, mnsCurveZipNode.node.BToMid])
		...
		conditionNodeA = mnsNodes.conditionNode(upperLidCtrl.node.tz, 0.0, [setRangeNodeBelowZero.node.outValueX, 0.0, 0.0], [setRangeNode.node.outValueX, 0.0, 0.0], [mnsCurveZipNode.node.AToMid, None, None], operation = 4)
		conditionNodeB = mnsNodes.conditionNode(lowerLidCtrl.node.tz, 0.0, [setRangeNodeBelowZero.node.outValueY, 0.0, 0.0], [setRangeNode.node.outValueY, 0.0, 0.0], [mnsCurveZipNode.node.BToMid, None, None], operation = 4)
		...
		pm.delete(pm.pointConstraint(lowerLidOffsetGrp.node, upperLidOffsetGrp.node, sk = ["z", "y"]))
		...
		ctrlsCollect.append(frameCtrl)
		pm.delete(pm.pointConstraint(cornerA, innerLidOffsetGrp.node))
		pm.parent(innerLidOffsetGrp.node, uiCtrlsGrp.node)
		...
		ctrlsCollect.append(frameCtrl)
		pm.delete(pm.pointConstraint(cornerB, outerLidOffsetGrp.node))
		pm.parent(outerLidOffsetGrp.node, uiCtrlsGrp.node)
		...
		if reverseCorners: outputArray = [mnsCurveZipNode.node.zipEnd, mnsCurveZipNode.node.zipStart]
		...
		for ctrlOffset in [upperLidOffsetGrp, innerLidOffsetGrp, lowerLidOffsetGrp]:
			pm.delete(pm.pointConstraint(outerLidOffsetGrp.node, ctrlOffset.node, sk = ["x", "y"]))

		#orient group
		if rootGuide.side == "r": 
			pm.delete(pm.aimConstraint(cornerA, uiCtrlsGrp.node, aimVector = (-1,0,0), upVector = (0,1,0), worldUpType = "object", worldUpObject = upperMid))
		else:
			pm.delete(pm.aimConstraint(cornerA, uiCtrlsGrp.node, aimVector = (1,0,0), upVector = (0,1,0), worldUpType = "object", worldUpObject = upperMid))
		
		#offset UIs
		status, uiCtrlsPositionOffset = mnsUtils.validateAttrAndGet(rootGuide, "uiCtrlsPositionOffset", 0.5)
		
		pm.move(uiCtrlsGrp.node, (0.0, 0.0, uiCtrlsPositionOffset), r = True, os = True, wd = True)
		
		if rootGuide.side == "r": 
			pm.move(uiCtrlsGrp.node, (-1.0 * uiCtrlsScale, 0.0, 0.0), r = True, os = True, wd = True)
		else:
			pm.move(uiCtrlsGrp.node, (1.0 * uiCtrlsScale, 0.0, 0.0), r = True, os = True, wd = True)
		...
		if doTweakControls:
			...
			for k, curve in enumerate(bindCurves):
				tempPocn = mnsNodes.mnsPointsOnCurveNode(side = rootGuide.side, 
														...
														aimAxis = 1,
														upAxis = 0,
														numOutputs = numTweakControlsPerSection)
				samplesToMatch = tempPocn["samples"]

				tweakControls = []
				for j, sampleToMatch in enumerate(samplesToMatch):
					isCorner = False
					if j == 0 or j == len(samplesToMatch) - 1: isCorner = True
					...
					#create control
					ctrlType = "ctrl"
					if k == 1 and isCorner: ctrlType = "techCtrl"

					tweakCtrl = blkCtrlShps.ctrlCreate(nameReference = rootGuide,
							color = blkUtils.getCtrlCol(rootGuide, MnsBuildModule.rigTop),
							matchTransform = sampleToMatch,
							ctrlType = ctrlType,
					...
					if not isCorner:
						if k == 0:
							upperTweaks.append(tweakCtrl)
						else:
							lowerTweaks.append(tweakCtrl)

					tweakControls.append(tweakCtrl)
					ctrlsCollect.append(tweakCtrl)
					
					if k == 1 and isCorner: 
						if j == 0: pm.parent(blkUtils.getOffsetGrpForCtrl(tweakCtrl).node, corners[0].node)
						else: pm.parent(blkUtils.getOffsetGrpForCtrl(tweakCtrl).node, corners[1].node)
						blkUtils.getOffsetGrpForCtrl(tweakCtrl).node.v.set(False)
				...
				#if hermite, kill midTweaker rotation
				if doTweakTangents and numTweakControlsPerSection == 3 and tweakCurvesInterpolation == 2:
					midRotKill = mnsUtils.createNodeReturnNameStd(parentNode = blkUtils.getOffsetGrpForCtrl(tweakControls[1]), side =  MnsBuildModule.rootGuide.side, body = MnsBuildModule.rootGuide.body + "midTweakRotMute", alpha = MnsBuildModule.rootGuide.alpha, id =  MnsBuildModule.rootGuide.id, buildType = "group", incrementAlpha = False)
					midRotKill.node.v.set(0)
					pm.delete(pm.parentConstraint(tweakControls[1].node, midRotKill.node))
					mnsNodes.mayaConstraint([tweakControls[1].node], midRotKill.node, type = "point")
					tweakControls = [tweakControls[0], midRotKill, tweakControls[2]]
				...
				status, doCornerTangents = mnsUtils.validateAttrAndGet(rootGuide, "doCornerTangents", False)
				if doTweakTangents and numTweakControlsPerSection == 3:
					if k == 0:
						for midTweakCtrl in upperTweaks: 
							createTangentsForCtrl(mansur, MnsBuildModule, midTweakCtrl, btcNode, corners[0], corners[1], "Upper", doCornerTangents, mainCtrl)
					else:
						for midTweakCtrl in lowerTweaks: 
							createTangentsForCtrl(mansur, MnsBuildModule, midTweakCtrl, btcNode, corners[0], corners[1], "Lower", doCornerTangents, mainCtrl)
				...
				if k == 0:
					btcNode["node"].node.outCurve >> mnsCurveZipNode.node.tweakCurveA
					curve.node.worldSpace[0] >> mnsCurveZipNode.node.tweakCurveABase
				else:
					btcNode["node"].node.outCurve >> mnsCurveZipNode.node.tweakCurveB
					curve.node.worldSpace[0] >> mnsCurveZipNode.node.tweakCurveBBase

				pm.delete(tempPocn["samples"])
				pm.delete(btcNode["outOffsetCurve"].node)


			status, extraTweakersUnderMid = mnsUtils.validateAttrAndGet(rootGuide, "extraTweakersUnderMid", True)
			if extraTweakersUnderMid and numTweakControlsPerSection == 5:
				for tweakGroup in [upperTweaks, lowerTweaks]:
					pm.parent(blkUtils.getOffsetGrpForCtrl(tweakGroup[0]).node, tweakGroup[1].node)
					parentCns = mnsNodes.mayaConstraint([corners[0].node, tweakGroup[1].node], blkUtils.getOffsetGrpForCtrl(tweakGroup[0]), type = "parent", maintainOffset = True)
					mnsUtils.lockAndHideTransforms(tweakGroup[0].node, tx = False, ty = False, tz = False, lock = True)
					
					pm.parent(blkUtils.getOffsetGrpForCtrl(tweakGroup[2]).node, tweakGroup[1].node)
					parentCns = mnsNodes.mayaConstraint([corners[1].node, tweakGroup[1].node], blkUtils.getOffsetGrpForCtrl(tweakGroup[2]).node, type = "parent", maintainOffset = True)
					mnsUtils.lockAndHideTransforms(tweakGroup[2].node, tx = False, ty = False, tz = False, lock = True)

		#blinkCtrl
		status, doBlinkTweak = mnsUtils.validateAttrAndGet(rootGuide, "doBlinkTweak", True)
		if doBlinkTweak:
			status, blinkCtrlShape = mnsUtils.validateAttrAndGet(rootGuide, "blinkTweakControlShape", "diamond")

			#create control
			blinkCtrl = blkCtrlShps.ctrlCreate(nameReference = rootGuide,
												color = blkUtils.getCtrlCol(rootGuide, MnsBuildModule.rigTop),
												bodySuffix = "MidCurveTweak",
												controlShape = blinkCtrlShape,
												scale = modScale * 0.1, 
												parentNode = mainCtrl,
												symmetryType = symmetryType,
												createSpaceSwitchGroup = False,
			...
			#get blink ctrl position
			upperLidCtrl.node.tz.set(2)
			lowerLidCtrl.node.tz.set(2)
			midCurve = pm.createNode("nurbsCurve")
			midCurveUp = pm.createNode("nurbsCurve")

			mnsCurveZipNode.node.outCurveA >> midCurve.create
			mnsCurveZipNode.node.outCurveAOffset >> midCurveUp.create
			
			midCurve.create.disconnect()
			midCurveUp.create.disconnect()
			upperLidCtrl.node.tz.set(0)
			lowerLidCtrl.node.tz.set(0)

			midCurve = midCurve.getParent()
			midCurveUp = midCurveUp.getParent()

			tempPocn = mnsNodes.mnsPointsOnCurveNode(inputCurve = midCurve,
											inputUpCurve = midCurveUp,
											buildOutputs = False,
											buildMode = 0,
											aimAxis = 1,
											upAxis = 0,
			...
			tempPocn["node"].node.transforms[1].rotate >> tempLoc.r
			tempLoc.t.disconnect()
			tempLoc.r.disconnect()
			pm.delete(pm.parentConstraint(tempLoc, blkUtils.getOffsetGrpForCtrl(blinkCtrl).node))
			pm.delete([tempLoc, midCurve, midCurveUp])
			

			midBtc = mnsNodes.mnsBuildTransformsCurveNode(
									side = rootGuide.side, 
									alpha = rootGuide.alpha, 
									id = rootGuide.id, 
									body = rootGuide.body + "MidCurve", 
									transforms = [corners[0], blinkCtrl, corners[1]], 
									deleteCurveObjects = True,  
									buildOffsetCurve = True,
									buildMode = 0,
									degree = 3,
									...
									offsetY = offsetY,
									offsetZ = offsetZ)
			midBtc["node"].node.outCurve >> mnsCurveZipNode.node.midCurve
			
			createTangentsForCtrl(mansur, MnsBuildModule, blinkCtrl, midBtc, corners[0], corners[1], "Mid", False, mainCtrl)
	...
	for inputName in inputNames:
		...
		if origAttachPoc:
			...
			uScaleInvAttr = mnsUtils.addAttrToObj([attachmentCtrlsDict[inputName + "AttachmentAll"]["ctrl"].node], type = "float", value = tugScaleValue, name = "uShift", cb = False, keyable = False, locked = True, replace = True)[0]
	...
	status, blinkHeight = mnsUtils.validateAttrAndGet(rootGuide, "midCurveHeight", 0.5)
	...
	if not doBlinkTweak:
		generateMidCurveFromAttr = mnsUtils.addAttrToObj([mainCtrl.node], type = "enum", value = ["baseCurves", "tweakedCurves"], name = "generateMidCurveFrom", replace = True)[0]
		generateMidCurveFromAttr >> mnsCurveZipNode.node.midGenerateFrom
	else:
		mnsCurveZipNode.node.midGenerateFrom.set(2)
	...
	conformToMeetAttr = mnsUtils.addAttrToObj([mainCtrl.node], type = "bool", value = True, name = "conformToMeetPoint", replace = True)[0]
	...
	return ctrlsCollect, internalSpacesDict, mainCtrl
